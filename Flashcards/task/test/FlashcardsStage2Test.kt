    ...
    override fun check(reply: String, clue: FlashcardsStage2Clue): CheckResult {
        ...
        if ((hasRight || !hasWrong) && clue.answer == WRONG) {
            return if (clue.revealTest) {
                CheckResult(
                        false,
                        revealRawTest(clue.consoleInput, reply) + "Did not find the \"${WRONG.word}\" word " +
                                "or encountered the \"${RIGHT.word}\" word."
                )
            } else {
                CheckResult(false)
            }
        ...
        if ((hasWrong || !hasRight) && clue.answer == RIGHT) {
            return if (clue.revealTest) {
                CheckResult(
                        false,
                        revealRawTest(clue.consoleInput, reply) + "Did not find the \"${RIGHT.word}\" word " +
                                "or encountered the \"${WRONG.word}\" word."
                )
            } else {
                CheckResult(false)
            }
